...
void ULyraEquipmentInstance::SpawnEquipmentActors(const TArray<FLyraEquipmentActorToSpawn>& ActorsToSpawn)
{
	if (APawn* OwningPawn = GetPawn())
	{
		...
		for (const FLyraEquipmentActorToSpawn& SpawnInfo : ActorsToSpawn)
		{
			...
			NewActor->AttachToComponent(GetAttachTarget(NewActor), FAttachmentTransformRules::KeepRelativeTransform, SpawnInfo.AttachSocket);
...
USceneComponent* ULyraEquipmentInstance::GetAttachTarget(AActor* NewActor) const
{
